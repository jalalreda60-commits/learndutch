...
export default function GrammarScreen() {
  ...
  const checkExercise = (exId: string, optIdx: number, correct: number) => {
              ...
              <View key={ex.id} style={styles.exCard}>
                    ...
                    {ex.options.map((opt, i) => {
                      ...
                      return (
                        <TouchableOpacity
                          key={i}
                          disabled={answered}
                          onPress={() => checkExercise(ex.id, i, ex.correct!)}
                ...
                {answered && (
                  <Text style={[styles.exFeedback, { color: userAnswer === ex.correct ? Colors.green : Colors.accent2 }]}>
                    {userAnswer === ex.correct ? '🎉 Correct !' : `❌ Réponse: "${ex.options?.[ex.correct!]}"`}
                  </Text>
                )}
              </View>
            );
